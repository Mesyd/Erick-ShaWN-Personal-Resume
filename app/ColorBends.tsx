...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      const x = (event.clientX / (window.innerWidth || 1)) * 2 - 1;
      const y = -((event.clientY / (window.innerHeight || 1)) * 2 - 1);
    ...
    const loop = () => {
      ...
      const amount = Math.min(1, dt * 6);
      pointerCurrentRef.current.lerp(pointerTargetRef.current, amount);
  ...
  useEffect(() => {
    ...
    material.uniforms.uIterations.value = Math.min(5, Math.max(1, iterations));
